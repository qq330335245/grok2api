...
func New(ctx context.Context, cfg config.Config, logger *slog.Logger) (*Application, error) {
	...
	trafficRecorder := trafficlog.New(trafficlog.Config{Enabled: cfg.Audit.TrafficLogEnabled, Directory: cfg.Audit.TrafficLogDirectory, MaxFiles: cfg.Audit.TrafficLogMaxFiles}, logger)
	...
	settingsService := settingsapp.NewService(cfg, settingsUpdatedAt, settingsRevision, runtimeSettingsRepo, notifySettings, func(next config.Config) {
		...
		antiDegradeController.Update(antiDegradeRuntime(next.QualityGuard.AntiDegrade))
		...
		trafficRecorder.Update(trafficlog.Config{Enabled: next.Audit.TrafficLogEnabled, Directory: next.Audit.TrafficLogDirectory, MaxFiles: next.Audit.TrafficLogMaxFiles})
