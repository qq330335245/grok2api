  ...
  "zh-CN": {
    translation: {
      ...
      docs: {
        ...
        endpointVideoEdit: "按官方 /v1/videos/edits 修改已有视频（Console；当前模型 grok-imagine-video）。",
        endpointVideoExtend: "按官方 /v1/videos/extensions 从末帧续写（duration 为扩展段 2–10 秒）。",
        endpointVideoGet: "按 request_id 查询 pending、done 或 failed 状态、progress 及生成结果。",
        streaming: "流式响应",
        ...
        reference: {
          ...
          fieldVideoEditPrompt: "描述如何修改画面（编辑接口必填）。",
          fieldVideoExtendPrompt: "描述续写镜头与动作（扩展接口必填）。",
          fieldSourceVideo: "源视频对象，提供 url 或 file_id（官方 video 字段）。",
          fieldExtendDuration: "扩展段时长，2–10 秒，默认 6（不是成片总时长）。",
          ...
          noteVideoStatus: "响应状态为 pending、done 或 failed，可含 progress（0–100）；完成后 video 中包含结果信息。",
          noteVideoEdit: "编辑不支持自定义 duration/aspect_ratio/resolution；输出跟随源视频（分辨率封顶 720p）。image 与 reference_images 不可用于本接口。",
          noteVideoExtend: "扩展 duration 表示新增片段秒数（2–10）。当前 Console 路径仅 grok-imagine-video。",
  ...
  en: {
    translation: {
      ...
      docs: {
        ...
        endpointVideoEdit: "Edit an existing video via official /v1/videos/edits (Console; model grok-imagine-video).",
        endpointVideoExtend: "Extend a video via /v1/videos/extensions (duration is the 2–10s extension segment).",
        endpointVideoGet: "Poll pending, done, or failed status, progress, and the result by request_id.",
        streaming: "Streaming",
        ...
        reference: {
          ...
          fieldVideoEditPrompt: "How to modify the clip (required for edits).",
          fieldVideoExtendPrompt: "How the motion should continue (required for extensions).",
          fieldSourceVideo: "Source video object with url or file_id (official video field).",
          fieldExtendDuration: "Extension segment length, 2–10 seconds, default 6 (not total output length).",
          ...
          noteVideoStatus: "Status is pending, done, or failed and may include progress (0–100). Completed responses include result details in video.",
          noteVideoEdit: "Edits do not take custom duration/aspect_ratio/resolution; output follows the source (resolution capped at 720p). Do not send image or reference_images.",
          noteVideoExtend: "Extension duration is the added segment in seconds (2–10). Console path currently supports grok-imagine-video only.",
