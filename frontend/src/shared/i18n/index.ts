...
const resources = {
  "zh-CN": {
    translation: {
      ...
      antidegrade: {
        title: "反降智", description: "按 ExitIP 密度与明文思考隔离降智账号，并手操冷却。",
        loadTab: "IP 负载", quarantineTab: "隔离", settingsTab: "设置",
        overview: "反降智概览", serviceStatus: "服务状态", running: "运行中", stopped: "已关闭",
        scopeHelp: "未勾选的渠道原样转发", coolingHelp: "密度窗口内冷却中的 ExitIP", quarantineHelp: "换口后仍连续无思考被隔离的账号；累犯会加长隔离且档案保留",
        policy: "当前策略", editPolicy: "编辑策略",
        summary: "{{mode}} · {{channels}} · 窗口 {{window}} · 每口 {{cap}} · 失败出口 {{threshold}}",
        modes: { observe: "观察", enforce: "执行" },
        loadHelp: "数字是密度窗口内打过该出口的去重账号数，不是实时连接数。粘性节点按账号分行，固定口按 ExitIP 列出。",
        exitIp: "出口 IP", nodes: "节点", load: "负载", accounts: "账号", score: "质量分", cooldown: "冷却",
        cooling: "冷却中", idle: "空闲", full: "已满", nearFull: "接近上限", override: "人工覆盖",
        noIps: "当前没有窗口负载或冷却中的出口。", filterAll: "全部", filterOk: "正常", groupSummary: "{{count}} 个号 · {{cooling}} 冷却", fixedExits: "固定出口",
        quarantined: "隔离中的账号", noQuarantine: "没有隔离中的账号。",
        failedIps: "失败出口", recidivism: "累犯", until: "至 {{time}}", clearAccount: "解除隔离", clearIp: "解除冷却",
        detectBotFlag: "检测风控", testVideo: "测试视频", testVideoOpen: "已打开创作台生成视频（{{count}} 个号）",
        clearedAccount: "已解除账号隔离", clearedAccounts: "已解除 {{count}} 个隔离", disabledAccounts: "已停用 {{count}} 个账号", clearedIp: "已解除出口冷却",
        events: "最近事件", noEvents: "暂无事件", eventOk: "有思考", eventMiss: "无思考",
        reasons: { dirty_ip: "脏口", farm: "农场口", density: "密度", k_ip_no_streamed_thinking: "失败出口达阈值", consecutive_ip_no_thinking: "换口连续无思考", egress_down: "出口故障" },
        enabled: "启用", providers: "作用范围", providersHelp: "默认只监控 Build。打开 Console / Web 后，那些渠道的无思考请求才会被 hold 并隔离。",
        providerNames: { grok_build: "Build", grok_console: "Console", grok_web: "Web" },
        models: "模型", modelsHelp: "默认全部开启。关掉的模型不再 hold、换口或隔离；之后新增的模型仍默认开启。",
        modelsAll: "全选 {{count}} 个", modelsEmpty: "还没有模型路由。",
        mode: "模式", densityWindow: "密度窗口", densityMaxAccounts: "每口账号上限",
        failExitThreshold: "失败出口阈值", failExitHelp: "换到不同 ExitIP 后仍连续无明文思考的次数达到该值则隔离。同一口连跪只冷口。成功思考会清零连续计数。",
        maxIpRetries: "同号最多换口次数", dirtyIpCooldown: "第一口冷却", farmIpCooldown: "农场口冷却",
        accountQuarantineTtl: "账号隔离时长", operatorOverride: "手开后保护", thinkingMinOutput: "可见输出阈值",
        advanced: "探索参数", scorePrior: "质量分先验", exploreRatio: "探索比例",
        save: "保存设置", saved: "设置已保存并立即生效",
        saveHint: "改窗口/上限立即作用于下一枪调度。已在冷却或隔离中的到期时间不会被改写。",
        conflict: "设置已被其他会话更新，请刷新后重试。",
      },
      qualityGuard: {
      ...
      models: {
        ...
        selectAllAccounts: "全选当前 {{count}} 个",
      ...
      settings: {
        ...
        audit: {
          ...
          trafficLogTitle: "请求/响应调试日志",
          trafficLogEnabled: "记录完整请求与上游 SSE",
          trafficLogEnabledHelp: "打开后把每次推理的请求头（Authorization 等会脱敏）、请求体和上游流式响应写成 data/traffic-logs 下的文件，格式接近 CPA。体积很大，只在排查 Hold/降智时打开，用完关掉。",
          trafficLogMaxFiles: "最多保留文件数",
          trafficLogMaxFilesHelp: "超过该数量时自动删除最旧的调试日志，防止忘关开关把磁盘写满。默认 50，范围 1–500。",
        ...
        accounts: {
          ...
          excludeBuildBotFlaggedFromSchedulingHelp: "开启后，风控标志为 1 或 2 的 Grok Build 账号不再参与 Build 渠道调度。检测风控会把同样的标志写到关联 Web / Console，但这两边调度仍不受此开关影响。账号仍保留在池中，可手动查看与操作。",
          buildBotRiskProbeModel: "风控检测模型",
          buildBotRiskProbeModelHelp: "手动检测风控，以及反降智在隔离后做号级确认时，用这个模型发送思考请求。可选 grok-4.5、grok-4.6、grok-4.7，默认 grok-4.5。",
          cleanupTitle: "账号清理",
      ...
      docs: {
        ...
        reference: {
          ...
          fieldEditMask: "可选 OpenAI 风格蒙版，JSON 字段 mask.url（透明像素=编辑区域）。Web 路由会转成 Imagine selectionRegions；不要与 selection_regions 同时传。",
          fieldSelectionRegions: "可选 Grok 原生多边形选区，points 为 0–1 归一化 [x,y,...]。与 mask 互斥。",
          ...
          noteEditMask: "局部编辑仅 Web/grok-imagine-image-edit 支持：传 mask.url 由网关转多边形，或直接传 selection_regions。Console 图片编辑会拒绝这两个字段。",
  ...
  en: {
    translation: {
      ...
      nav: { dashboard: "Dashboard", accounts: "Accounts", models: "Models", clientKeys: "Client keys", creativeConsole: "Creative console", gallery: "Gallery", videoGallery: "Video Gallery", audits: "Request audits", qualityGuard: "Quality guard", antidegrade: "Anti-degrade", docs: "Documentation", settings: "Settings" },
      creativeConsole: {
      ...
      antidegrade: {
        title: "Anti-degrade", description: "Quarantine degraded accounts by ExitIP density and streamed thinking, with manual cooldowns.",
        loadTab: "IP load", quarantineTab: "Quarantine", settingsTab: "Settings",
        overview: "Anti-degrade overview", serviceStatus: "Service", running: "Running", stopped: "Off",
        scopeHelp: "Unchecked channels pass through unchanged", coolingHelp: "ExitIPs cooling in the density window", quarantineHelp: "Accounts isolated after consecutive missing-thinking on switched ExitIPs; recidivism lengthens the lock and is kept after release",
        policy: "Current policy", editPolicy: "Edit policy",
        summary: "{{mode}} · {{channels}} · window {{window}} · cap {{cap}} · fail-exit {{threshold}}",
        modes: { observe: "Observe", enforce: "Enforce" },
        loadHelp: "Counts are unique accounts in the density window, not live connections. Sticky nodes are grouped by account; dedicated nodes list ExitIPs.",
        exitIp: "Exit IP", nodes: "Nodes", load: "Load", accounts: "Accounts", score: "Score", cooldown: "Cooldown",
        cooling: "Cooling", idle: "Idle", full: "Full", nearFull: "Near cap", override: "Operator override",
        noIps: "No ExitIPs with window load or an active cooldown.", filterAll: "All", filterOk: "Healthy", groupSummary: "{{count}} accounts · {{cooling}} cooling", fixedExits: "Dedicated exits",
        quarantined: "Quarantined accounts", noQuarantine: "No quarantined accounts.",
        failedIps: "Failed exits", recidivism: "Recidivism", until: "Until {{time}}", clearAccount: "Lift quarantine", clearIp: "Lift cooldown",
        detectBotFlag: "Detect bot risk", testVideo: "Test video", testVideoOpen: "Opened the creative console for video ({{count}} accounts)",
        clearedAccount: "Account quarantine cleared", clearedAccounts: "Lifted quarantine for {{count}} accounts", disabledAccounts: "Disabled {{count}} accounts", clearedIp: "ExitIP cooldown cleared",
        events: "Recent events", noEvents: "No events", eventOk: "Thinking", eventMiss: "No thinking",
        reasons: { dirty_ip: "Dirty IP", farm: "Farm IP", density: "Density", k_ip_no_streamed_thinking: "Fail-exit threshold", consecutive_ip_no_thinking: "Consecutive missing thinking", egress_down: "Egress down" },
        enabled: "Enabled", providers: "Scope", providersHelp: "Build only by default. Turn on Console / Web to hold and quarantine those channels too.",
        providerNames: { grok_build: "Build", grok_console: "Console", grok_web: "Web" },
        models: "Models", modelsHelp: "Every model is on by default. A model that is off is not held, retried, or quarantined. Models added later stay on.",
        modelsAll: "Select all {{count}}", modelsEmpty: "No model routes yet.",
        mode: "Mode", densityWindow: "Density window", densityMaxAccounts: "Accounts per ExitIP",
        failExitThreshold: "Fail-exit threshold", failExitHelp: "Consecutive missing-thinking after switching ExitIP. Same-IP repeats only cool the IP. Streamed thinking resets the streak.",
        maxIpRetries: "Max ExitIP retries per account", dirtyIpCooldown: "First-IP cooldown", farmIpCooldown: "Farm-IP cooldown",
        accountQuarantineTtl: "Account quarantine TTL", operatorOverride: "Manual-open protection", thinkingMinOutput: "Visible output threshold",
        advanced: "Explore", scorePrior: "Score prior", exploreRatio: "Explore ratio",
        save: "Save settings", saved: "Settings saved and applied immediately",
        saveHint: "Window and cap apply to the next request. Existing cooldown and quarantine deadlines are not rewritten.",
        conflict: "Settings were updated in another session. Refresh and retry.",
      },
      ...
      docs: {
        ...
        reference: {
          ...
          fieldEditMask: "Optional OpenAI-style mask as mask.url (transparent pixels are edited). Web routes convert it to Imagine selectionRegions. Do not send it together with selection_regions.",
          fieldSelectionRegions: "Optional native Grok polygon regions. points are normalized 0–1 [x,y,...] values. Mutually exclusive with mask.",
          ...
          noteEditMask: "Region edits are Web/grok-imagine-image-edit only: send mask.url for gateway polygon conversion, or pass selection_regions directly. Console image edits reject both fields.",
...
Object.assign(resources["zh-CN"].translation.accounts as unknown as Record<string, unknown>, {
  ...
  detectBotFlagAllDescription: "对每个 Build 号用粘性 {account}+n 换口打思考请求：全部无思考视为号级风控，并同步打到关联的 Web / Console。不走 grok-4.5 探活，不标失效，不需要 SSO。",
  detectBotFlagSelectedTitle: "检测选中的 {{count}} 个账号风控？",
  detectBotFlagSelectedDescription: "对选中的 Build 号用粘性 {account}+n 换口打思考请求：全部无思考视为号级风控，并同步打到关联的 Web / Console。不走 grok-4.5 探活，不标失效，不需要 SSO。",
  ...
  detectVerdict: { thinking: "有思考", missing: "无思考", inconclusive: "未决" },
  detectBotFlagThinking: "有思考 · {{event}}",
  detectBotFlagThinkingPlain: "有思考",
  detectBotFlagNoThinking: "{{count}} 个粘性出口均无思考",
  batchDisableTitle: "停用 {{count}} 个账号？",
  batchDisableDescription: "停用后不再参与调度，可随时重新启用。",
  linkedDisableTitle: "同时停用关联账号？",
  linkedDisableHint: "只停用已勾选的关联账号；未勾选的保持原状。",
});
Object.assign(resources.en.translation.accounts as unknown as Record<string, unknown>, {
  ...
  detectBotFlagAllDescription: "Probe each Build account on sticky {account}+n exits. No streamed thinking on every attempt means account-level bot risk and is copied to linked Web / Console accounts. This does not run the grok-4.5 liveness probe, mark credentials invalid, or require SSO.",
  detectBotFlagSelectedTitle: "Detect bot risk for {{count}} selected accounts?",
  detectBotFlagSelectedDescription: "Probe selected Build accounts on sticky {account}+n exits. No streamed thinking on every attempt means account-level bot risk and is copied to linked Web / Console accounts.",
  ...
  detectVerdict: { thinking: "Thinking", missing: "No thinking", inconclusive: "Inconclusive" },
  detectBotFlagThinking: "Thinking · {{event}}",
  detectBotFlagThinkingPlain: "Thinking",
  detectBotFlagNoThinking: "No thinking on {{count}} sticky exits",
  batchDisableTitle: "Disable {{count}} accounts?",
  batchDisableDescription: "Disabled accounts leave scheduling and can be re-enabled later.",
  linkedDisableTitle: "Also disable linked accounts?",
  linkedDisableHint: "Only checked linked accounts are disabled; others stay unchanged.",
});
Object.assign(resources.en.translation.accounts as unknown as Record<string, string>, {
  freeConfirmedUsage: "{{used}} / {{limit}} tokens",
});
